...
export function ListRatings({ userId }: Props) {
  ...
  async function fetchData(pageNumber = 1, user: string | undefined = undefined) {
    const { ratings, count } = await fetchWrapper<{ ratings: DataCard[] | null, count: number }>(`ratings/last-ratings?userId=${user}&page=${pageNumber}`, {
      method: 'GET'
    })
  ...
  return (
    <div id="scrollId" className="flex max-h-full  gap-4 overflow-auto">
      <InfiniteScroll
